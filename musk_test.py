...
def main():
    parser = argparse.ArgumentParser(description='Train bag_mil on MUSK datasets')
    parser.add_argument('--datasets', default='musk1', type=str, help='Choose MIL datasets from: musk1, musk2, [musk1]')
    ...
    parser.add_argument('--model', default='bag_mil', type=str, help='MIL model [bag_mil]')
    args = parser.parse_args()
    
    if args.model == 'bag_mil':
        import bag_mil as mil
    
    if args.datasets == 'musk1':
        data_all = get_data('MUSK1/musk1norm.svm')
        args.num_feats = 166
...
#        args.num_feats = 166
    ...
    print('Accuracy: %.4f, std %.4f ' % (np.mean(np.array(acs)), np.std(np.array(acs))))
